...
| Field | Value |
|---|---|
| Report | heldout_r0_f1guard_equal |
| Date | 2026-09-26 06:39 |
| Node | local (macOS, CPU) |
| Track | A-dev (HB-GP stand-in final cost; bridge guard at f1, equal f1 guard for the other partners) |
| Tool versions | HeurBridge 0.10.1 |
...
| memetic | 0.5871 | 0.4315 | 0.9750000000000001 | 0.53 | 2.12e-05 | 4.25e-05 |
| none | 0.5910 | 0.4321 | 1.0 | 0.55 | 1.23e-07 | 3.7e-07 |
| repertoire | 0.5782 | 0.4321 | 1.0 | 0.48 | 0.00524 | 0.00524 |
...
```bash
python scripts/run_e0.py --suite ibm --designs ibm04,ibm06 --runs ./runs/seed_dev --bridge checkpoints/bridge_dev_r0/best.pt --final hbgp --seeds 2 --K 20 --out runs/e0_dev/heldout_r0_f1guard_equal --campaign E0_dev --guard-fidelity f1 --equal-guard True
```

## Notes

raw rows: runs/e0_dev/heldout_r0_f1guard_equal/e0_rows.jsonl. Wins / losses count paired cases where the partner's final J is below / above the raw layout's; the geometric-mean ratio < 1 means lower J on average in log terms.
